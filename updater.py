...
def stashChanges(repo):
    ...
    choice = input("Would you like to stash local changes? (Required to continue) [Y/n]: ")
    if any(choice == x for x in ["", "yes", "Yes", "YES", "yes", "y", "Y"]):
        ...
        try:
            ...
            userName = input("--> Please enter a global username for git on this system (just make something up): ")
        ...
        try:
            ...
            userEmail = input("--> Please enter a global user e-mail address for git on this system: ")
...
def check_repo(repo):
    ...
    if not localBranch in ["master", "legacy"] and not userInput:
        ...
        choice = input("It is highly recommended that you switch to the stable master branch."
    ...
    if userInput and len(repo.remotes) > 1:
        ...
        while 1:
            try:
                choice = input("From which remote do you want to update? [%s]:" % remote)
                if choice == "":
    ...
    if userInput:
        ...
        while 1:
            try:
                choice = input("Enter the number of the branch you wish to update [%s]:" % localBranch)
                if choice == "":
    ...
    if localBranch != remoteBranch:
        choice = input("The " + remoteBranch + " branch is not your currently active branch - " +
    ...
    if localDate < remoteDate:
        ...
        choice = input("Would you like to update " + localName + " from " + remoteName + " [Y/n]: ")
        if any(choice == x for x in ["", "yes", "Yes", "YES", "yes", "y", "Y"]):
...
for i in range(3):
    ...
    try:
        ...
        scriptPath = input("What path did you install the BrewPi python scripts to?")
        continue
...
for i in range(3):
    ...
    try:
        ...
        webPath = input("What path did you install the BrewPi web interface scripts to? ")
        continue
    except (git.InvalidGitRepositoryError, IOError):
